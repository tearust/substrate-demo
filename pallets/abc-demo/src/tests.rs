...
fn generate_errand_id_test() {
    new_test_ext().execute_with(|| {
        ...
        let errand_id = TemplateModule::generate_errand_id(&AccountId32::from_string(ACCOUNT1).unwrap());
...
fn begin_task_works() {
    ...
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(TemplateModule::request_delegate(sender.clone(), employer_account.clone()));
        assert_ok!(TemplateModule::update_delegate_status(sender.clone(), employer_account.clone(), sender_account.clone()));
        assert_ok!(TemplateModule::begin_task(sender.clone(), employer_account.clone(), cid.clone(), fee));
        ...
        assert_eq!(&AccountId32::from_string(ACCOUNT1).unwrap(), &AccountId32::from(sender_bytes));
        assert_eq!(&AccountId32::from_string(ACCOUNT2).unwrap(), &AccountId32::from(employer_bytes));
        ...
        assert_eq!(&AccountId32::from_string(ACCOUNT1).unwrap(), &AccountId32::from(sender_bytes_2));
        assert_eq!(&cid, &task_array2[0].description_cid);
        assert_eq!(&fee, &task_array2[0].fee);

        let errand_id = vec![3u8, 4u8];
        assert_ok!(TemplateModule::init_errand(sender.clone(), employer_account.clone(), errand_id.clone(), cid.clone()));
